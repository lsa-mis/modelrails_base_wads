...
RSpec.describe CheckGravatarJob, type: :job do
  ...
  it "updates has_gravatar to true when Gravatar exists" do
    ...
    allow(job).to receive(:gravatar_exists?).with(user.email_address).and_return(true)
    job.perform(user)
  ...
  it "updates has_gravatar to false when Gravatar does not exist" do
    ...
    job = described_class.new
    allow(job).to receive(:gravatar_exists?).with(user.email_address).and_return(false)
    job.perform(user)
